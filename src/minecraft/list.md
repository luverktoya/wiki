...
- [Purpur](https://purpurmc.org/) - форк Paper, то есть включает в себя поддержку всех оптимизаций Paper.
  Плагины, работающие на Paper, будут работать и на его форках **(за исключением Folia)**. Purpur добавляет множество настроек, которые дают
...
- [Folia](https://papermc.io/software/folia/) - форк Paper с региональной многопоточностью. Вместо одного ядра грузит все доступные ядра CPU, обрабатывая разные части карты параллельно. **Ломает совместимость** с большинством плагинов. Подходит **только** для **очень крупных** проектов, где обычный Paper уже не справляется с нагрузкой.
